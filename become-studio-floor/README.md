...
## Bubble lines people write themselves

Create a public channel called **#studio-floor-lines** and invite the app to it (`/invite @Studio Floor`).
Anything a person posts there becomes their own speech-bubble lines on the floor: one line per
message line, up to 48 characters each, latest 6 kept. Deleting the message removes the lines within 10 minutes.
Use a different channel name with the `LINES_CHANNEL` environment variable.
