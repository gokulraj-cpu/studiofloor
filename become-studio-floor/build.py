...
function askPasscode(wrong){
  pollers.forEach(id => { clearInterval(id); clearTimeout(id); }); pollers = [];
  banner.innerHTML = "";
...
async function startStandalone(){
  ...
  pollers.forEach(id => { clearInterval(id); clearTimeout(id); });
  // checks run on the clock (every :00 and :30), so every open screen sees a change at the same moment
  const onClock = (ms, fn, offset) => { const h = {id: null}; h.id = setTimeout(() => { fn(); h.id = setInterval(fn, ms); pollers.push(h.id); }, ms - (Date.now() % ms) + offset); return h.id; };
  pollers = [
    onClock(30000, () => api("/api/office").then(x => applyOffice(x, false)).catch(onPollError), 1200),
